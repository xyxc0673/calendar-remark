...
const DateSelector = ({
  ...
  return (
    <div className='relative group before:absolute before:left-0 before:right-0 before:top-0 before:block before:leading-tight before:cursor-pointer before:text-lg before:content-[""] before:-bottom-1'>
      <div className='flex items-center py-1 pl-3 pr-1 transition-colors duration-200 bg-white border border-transparent rounded cursor-pointer hover:border-gray-600'>
        <span className='group-hover:scale-100'> {render(value)}</span>
        <ChevronDown className='w-6 h-6 ml-1 text-gray-500 transition-transform duration-200 group-hover:rotate-180' />
      </div>
      <div
        ref={listRef}
        className='absolute z-50 flex flex-col invisible w-full gap-1 p-1 overflow-hidden overflow-y-scroll transition-all duration-300 translate-y-0 bg-white rounded shadow opacity-0 h-96 shadow-slate-200 group-hover:translate-y-1 top-full group-hover:opacity-100 group-hover:visible scrollbar-track-white scrollbar-thumb-slate-300 scrollbar-thin scrollbar-thumb-rounded-full'
      >
        {valueList.map((item) => (
          <span
            ...
            key={item}
            className={clsxm(
              'text-zinc-500 opacity-80 hover:opacity-100 whitespace-nowrap hover:text-slate-900 inline-block w-full px-3 py-1 text-center border border-transparent rounded cursor-pointer hover:border-gray-600 transition-all duration-200',
...
const CalendarHeader = ({
  ...
  return (
    <div className='flex items-center justify-center gap-4 px-6 py-4 bg-slate-100'>
      <DateSelector
        valueList={yearList}
        value={currentYear}
